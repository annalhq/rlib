...
>(({ type = 'single', className, defaultValue, ...props }, ref) => {
  ...
  return (
    ...
    <AccordionPrimitive.Root
      ...
      collapsible
      className={cn(
        'mt-4 mb-4 mr-4 divide-y overflow-hidden rounded-lg border nx-bg-transparent nx-shadow-sm border-border dark:border-neutral-800 dark:divide-neutral-800', // Ensure border and divide colors match
        className,
      )}
      {...props}
    />
  ...
  return (
    ...
    >
      <AccordionPrimitive.Header
        id={id}
        className="not-prose flex flex-row items-center font-medium nx-text-gray-700 dark:nx-text-neutral-200"
      >
        <AccordionPrimitive.Trigger className="flex flex-1 items-center gap-2 p-4 text-start focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          <ChevronRight className="size-4  transition-transform duration-300 group-data-[state=open]/accordion:rotate-90" />
          {title}
        </AccordionPrimitive.Trigger>
        {id ? <CopyButton id={id} /> : null}
      </AccordionPrimitive.Header>
      <AccordionPrimitive.Content className="overflow-hidden data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down">
        <div className="ml-2 p-4 pt-0 prose-no-margin nx-bg-transparent dark:nx-bg-neutral-700">{children}</div>
